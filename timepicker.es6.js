...
export class Timepicker extends React.Component {
  ...
  onChange(hours, minutes) {
    ...
    if (this.props.onChange) this.props.onChange(hours, minutes)
  }

  onChangeMode(mode) {
    this.setState({ mode })

    if (this.props.onChangeMode) this.props.onChangeMode(mode)
  }
}
...
export class Info extends React.Component {
  ...
  render() {
    ...
    return (
      <p className="timepicker-info" style={{ width: size }}>
        <span
          className={`timepicker-info-digits${mode === Timepicker.HOURS
            ? " active"
            : ""}`}
        ...
        :
        <span
          className={`timepicker-info-digits${mode === Timepicker.MINUTES
            ? " active"
            : ""}`}
  ...
  onClickHours() {
    if (this.props.onChangeMode) this.props.onChangeMode(Timepicker.HOURS)
  }

  onClickMinutes() {
    if (this.props.onChangeMode) this.props.onChangeMode(Timepicker.MINUTES)
  }
}
...
export class Clock extends React.Component {
  ...
  componentWillReceiveProps({
    ...
    if (mode !== props.mode) this.setState({ mode: mode })

    if (hours !== props.hours)
      this.setState({ hours: hours % (militaryTime ? 24 : 12) })

    if (minutes !== props.minutes) this.setState({ minutes: minutes % 60 })
  }
  ...
  componentDidUpdate(previousProps, previousState) {
    ...
    hand1.setAttribute(
      "x2",
      mode
        ? positionsHours[hours === 0 ? (militaryTime ? 23 : 11) : hours - 1][0]
        : positionsMinutes[minutes][0]
    )
    hand1.setAttribute(
      "y2",
      mode
        ? positionsHours[hours === 0 ? (militaryTime ? 23 : 11) : hours - 1][1]
        : positionsMinutes[minutes][1]
    )
  ...
  render() {
    ...
    return (
      <svg height={size} width={size} ref={"foo"}>
        <line
          ref={el => (this.hand1 = el)}
  ...
  renderMinutesBubbles() {
    ...
    for (let minute = 0; minute < positionsMinutes.length; ++minute) {
      ...
      bubbles.push(
        <g
          className={`timepicker-bubble${minute % 5
            ? " small"
            : ""}${minutes === minute ? " active" : ""}`}
          key={minute}
          onClick={onClick}
          onMouseMove={onMouseMove}
        >
          <circle
            cx={x}
            cy={y}
            r={minute % 5 ? (minutes === minute ? 5 : 0) : 15}
          />
  ...
  onChange() {
    ...
    if (this.props.onChangeMode) this.props.onChangeMode(this.state.mode)
  }

  onClickHour(hours) {
    return (event, preventChangeMode) => {
      if (this.state.hours === hours && preventChangeMode) return
      ...
      this.setState(
        {
          ...
          mode: preventChangeMode
            ? this.state.mode === Timepicker.HOURS
              ? Timepicker.HOURS
              : Timepicker.MINUTES
            : this.state.mode === Timepicker.HOURS
              ? Timepicker.MINUTES
  ...
  onClickMinute(minutes) {
    return () => {
      if (this.state.minutes === minutes) return

      this.setState({ minutes: minutes, even: !this.state.even }, () =>
        this.onChange()
      )
    }
  }
  ...
  onMouseMoveHour(hours) {
    ...
    return function(event) {
      ...
      if (isMouseUp || event.buttons === 1) onClickHour(event, !isMouseUp)
    }
  }
  ...
  onMouseMoveMinute(minutes) {
    ...
    return function(event) {
      if (event.buttons === 1) onClickMinute()
    }
  }
    ...
    for (let index = 1; index <= (militaryTime ? 24 : 12); ++index) {
      positions.push([
        Math.round(
          size / 2 +
            radius *
              (militaryTime ? (index > 12 ? 1 : 0.65) : 1) *
